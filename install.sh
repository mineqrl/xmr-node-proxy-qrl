...
cd ~
git clone https://github.com/mineqrl/xmr-node-proxy-qrl.git
...
cd ~
pm2 status
sudo env PATH=$PATH:`pwd`/.nvm/versions/node/v8.14.0/bin `pwd`/.nvm/versions/node/v8.14.0/lib/node_modules/pm2/bin/pm2 startup systemd -u $CURUSER --hp `pwd`
